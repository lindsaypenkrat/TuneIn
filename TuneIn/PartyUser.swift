...
class PartyUser {
    //TODO: We need to extend the user class to allow for additional profile based information and nested data Ex. Musical Interest, Musical Instrument, Parties, Jams(these could be many relationships)
    var email: String
    ...
    func saveIfNewUser() {
        ...
        userRef.getDocument { (document, error) in
            ...
            guard document?.exists == false else {
                print("^^^ The document for user \(self.documentID) already exists. No reason to create it. Coming from user")
                return
            }
            self.saveData()
        }
